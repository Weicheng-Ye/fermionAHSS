...
def report(run):
    ...
    with (run/"resources.csv").open("w") as stream:
        fields = ["task", "group", "job_id", "state", "host", "wall_seconds",
                  ...
                  "kernel_max_rss_kib", "sampled_tree_peak_rss_kib", "cgroup_peak_bytes"]
    ...
    lines = [f"Updated {summary['updated_utc']}.", "",
             ...
             "Kernel ru_maxrss (or GNU time in reused initial tasks) is the largest process high-water mark; "
             "sampled tree RSS sums live descendants every 2 s. "
