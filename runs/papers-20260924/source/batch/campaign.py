...
def work(args):
    ...
    command = ["/usr/bin/time", "-v", "-o", str(directory/"time.txt"),
               manifest["gap"], "-q", "--quitonbreak", "-o", f"{gap_limit}m",
    ...
    import re
    time_text = (directory/"time.txt").read_text() if (directory/"time.txt").exists() else ""
    match = re.search(r"Maximum resident set size \(kbytes\): (\d+)", time_text)
    metrics["gnu_time_max_rss_kib"] = int(match[1]) if match else None
