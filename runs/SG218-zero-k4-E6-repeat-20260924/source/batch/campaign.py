...
def prepare(args):
    ...
    manifest = dict(created_utc=now(), package="fermionAHSS", certified_ko=False,
                    ...
                    partition=args.partition, memory_per_job_mib=args.memory_mib,
                    memory=(f"{args.memory_mib} MiB per job" if args.memory_mib else "all allocatable node memory (--mem=0)"),
...
def allocated_mib(requested_mib=0):
    if requested_mib:
        if requested_mib < 1:
            raise ValueError("job memory must be positive")
        return requested_mib
    # mem=0 does not reliably overwrite SLURM_MEM_PER_NODE inherited from the
    # submitting dispatcher. All campaign tasks request the full node, so use
    # the controller's RealMemory instead of that potentially stale variable.
...
def work(args):
    ...
    request_path = directory/"resource-request.json"
    request = json.loads(request_path.read_text()) if request_path.exists() else {}
    budget = allocated_mib(request.get("memory_mib", manifest.get("memory_per_job_mib", 0)))
...
def submission_environment():
    # Do not export a submitting job's resource variables into a new allocation.
...
def submit_task(run, manifest, task):
    ...
    command = ["sbatch", "--parsable", "--partition="+manifest["partition"],
               "--nodes=1", "--ntasks=1", "--cpus-per-task=1", "--mem="+str(manifest.get("memory_per_job_mib", 0))+"M",
               "--time="+manifest["time_limit"], "--signal=B:TERM@120",
               "--job-name=fahss-"+task["id"],
               "--output="+str(run/"slurm"/(task["id"]+"-%j.log")), str(script)]
    result = subprocess.run(command, text=True, capture_output=True, env=submission_environment())
...
def dispatch(args):
    ...
    while True:
        try:
            active = queue()
            # Keep one slot for renewing this dispatcher before its wall limit.
            capacity = max(0, manifest["max_submitted"]-1-len(active))
...
def submit(args):
    ...
    result = subprocess.run(command, capture_output=True, text=True, check=True, env=submission_environment())
...
def main():
    ...
    prep.add_argument("--memory-mib", type=int, default=6144, help="MiB per calculation; 0 requests all node memory")
    prep.add_argument("--time-limit", default="14-00:00:00")
    prep.add_argument("--max-submitted", type=int, default=150)
    prep.add_argument("--reuse-run", type=Path, help="reuse completed tasks with identical mathematical sources")
