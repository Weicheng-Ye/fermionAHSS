...
Current resource policy (user update, 2026-09-24): new preparations default to
6144 MiB and one CPU per calculation. `--memory-mib 0` restores the historical
full-node policy described below. To resize an existing run's queued tasks,
use `python3 batch/resize_pending.py RUN --memory-mib 6144`. This resubmits only
pending jobs, records old/new job IDs and a separate frozen resource controller,
and adjusts GAP's workspace cap to 85% of the per-job budget. Running tasks and
the original mathematical source snapshot are preserved.
...
The dispatcher respects 150 total submitted jobs for this user,
including unrelated jobs and itself, reserving one slot for dispatcher renewal.
It submits the next waiting task as
...
- Kernel `getrusage(RUSAGE_CHILDREN)` maximum RSS (KiB): largest process high-water mark, including
  inherited child resource usage, not the sum of simultaneous processes.
  Initial-run tasks used GNU time for the same quantity; one node lacked that
  executable, so the continuation uses Python resource accounting everywhere.
...
`prepare --reuse-run PRIOR` reuses completed tasks by
linking their task directories and preserving the prior manifest/source.
It first checks mathematical source hashes, references, page window, and task
inputs for equality. Instrumentation changes are allowed and remain explicit
in separate manifests. Do not delete the prior run while these links exist.
