...
and a finite horizontal perturbation. Trivial coefficient chains suffice for
binary sources; the two fibers have one common inversion action.
"""
...
def h_boundary(x):
    ...
    for i in range(p + 1) if p else ():
        y = h_pull(x, tuple(j for j in range(p + 1) if j != i))
